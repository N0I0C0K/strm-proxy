...
async def main() -> None:
    async with httpx.AsyncClient(
        ...
        movies = await XlysCatalog(client, limit=100).latest_movies()
    ...
    with TestClient(application, base_url="http://192.168.1.20:8787") as test_client:
        get_app_services(application).movie_repository.import_discovered(movies)
        ...
        assert listing.text.count("<D:response>") == 101
        ...
        assert "proxy_segments=true" in stream.text
    print("webdav_root_responses=3 (root + movie + series)", flush=True)
    print("webdav_movie_responses=101 (directory + 100 STRM files)", flush=True)
